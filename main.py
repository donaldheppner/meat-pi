...
class ThermistorReading:
    def __init__(self, thermistor, adc, resistance, voltage, temperature):
    ...
    def get_fahrenheit(self):
        return ((celcius(self) * 9) / 5) + 32

class Thermistor:
    # calculates the coefficients based on the calibartion points
    def __init__(self, calibration_points = [], resistor = 10000, voltage_in = 3.3):
    ...
    def get_reading(self, adc):
        # https://learn.adafruit.com/thermistor/using-a-thermistor
        resistance = self.resistor / ((65535 / adc) - 1)
        lnResistance = math.log1p(resistance)
        kelvins = 1 / (self.a + (self.b * lnResistance) + (self.c * (lnResistance ** 3)))

        return ThermistorReading(self, adc, resistance, voltage_out, kelvins)
...
class Cooker:
    ...
    def read_chamber(self):
        ...
        logging.debug(f'Read chamber value of: {chan.value} and voltage of {chan.voltage}')
